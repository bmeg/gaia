...
class GaiaJanus(config: GaiaGraphConfig) extends GaiaGraph {
  ...
  def retryCommit(times: Integer): Unit = {
    if (times == 0) {
      ...
      try {
        graph.asInstanceOf[JanusGraph].tx.commit()
      } catch {
        case ex: Exception => {
          retryCommit(times - 1)
        }
  ...
  def makeIndex(name: String) (keys: Map[String, Class[_]]): Try[Unit] = {
    Try {
      val mg = graph.asInstanceOf[JanusGraph].openManagement()
