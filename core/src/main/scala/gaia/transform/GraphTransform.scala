...
case class GraphTransform(graph: GaiaGraph) extends MessageTransform with GaiaIngestor {
  ...
  def associateEdges(graph: GaiaGraph) (vertex: Vertex) (edges: EdgeDescription) (field: Option[Any]): Unit = {    
    field.map { gids =>
      ensureSeq(gids).foreach { gid =>
        unembed(gid, edges.embeddedIn).map { in =>
          graph.associateOut(vertex, edges.edgeLabel, edges.destinationLabel, in)
        }
      }
    }
  }

  def linkThrough(graph: GaiaGraph) (gid: String) (vertex: Vertex) (link: EdgeDescription) (field: Option[Any]): Unit = {
    field.map { through =>
      ensureSeq(through).foreach { through =>
        unembed(through, link.embeddedIn).map { through =>
          val key = link.edgeLabel + through
          val existing = partialEdges.getOrElse(key, List[PartialEdge]())
          if (existing.isEmpty) {
            println(link.edgeLabel, through, link.destinationLabel)
            val partial = PartialEdge(from = Some(gid), edgeLabel = Some(link.edgeLabel))
            partialEdges += (key -> List(partial))
          } else {
            existing.foreach { exist =>
              println("linkThrough", gid, link.edgeLabel, link.destinationLabel, exist.to.get, exist.properties)
  ...
  def edgeSource(graph: GaiaGraph) (gid: String) (edge: EdgeDescription) (field: Option[Any]) (data: Map[String, Any]): Unit = {
    println("in edge source")
    field.map { source =>
      ensureSeq(source).foreach { source =>
        val sourceString = source.asInstanceOf[String]
        val key = edge.edgeLabel + gid // source.asInstanceOf[String]
        val existing = partialEdges.getOrElse(key, List[PartialEdge]())
        if (existing.isEmpty) {
          println(edge.edgeLabel, sourceString, edge.destinationLabel)
          addPartialEdge(key) (PartialEdge(edgeLabel = Some(edge.edgeLabel), from = Some(sourceString), fromLabel = Some(edge.destinationLabel), properties=data))
        } else {
          val from = findVertex(graph) (edge.destinationLabel) (sourceString)
          existing.foreach { exist =>
            println("edgeSource", sourceString, edge.edgeLabel, exist.toLabel.get, exist.to.get, exist.properties)
  ...
  def edgeTerminal(graph: GaiaGraph) (gid: String) (edge: EdgeDescription) (field: Option[Any]) (data: Map[String, Any]): Unit = {
    field.map { terminal =>
  ...
  def embeddedTerminals(graph: GaiaGraph) (gid: String) (edge: EdgeDescription) (field: Option[Any]) (data: Map[String, Any]): Unit = {
    field.map { terminal =>
      val terminals = ensureSeq(terminal)
      val key = edge.edgeLabel + gid
      val existing = partialEdges.getOrElse(key, List[PartialEdge]())
      if (existing.isEmpty) {
        ...
        terminals.foreach { terminal =>
          ...
          terminalMap.get(edge.embeddedIn).map { id =>
            addPartialEdge(key) (PartialEdge(edgeLabel = Some(edge.edgeLabel), to = Some(id.asInstanceOf[String]), toLabel = Some(edge.destinationLabel), properties=(data ++ terminalMap)))
          }
        }
      } else {
        existing.foreach { exist =>
          ...
          terminals.foreach { terminal =>
            val terminalMap = terminal.asInstanceOf[Map[String, Any]]
            terminalMap.get(edge.embeddedIn).map { id =>
              println("edgeTerminal", exist.from.get, edge.edgeLabel, edge.destinationLabel, id.asInstanceOf[String], exist.properties)
            }
            // graph.associateOut(from, edge.edgeLabel, edge.destinationLabel, terminalAsString, existing.get.properties)
          }
        }
